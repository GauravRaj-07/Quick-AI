...
const WriteArticle = () => {
  ...
  const [selectLength,setSelectedLength]=useState(articleLength[0])
  ...
  const onsubmitHandler= async (e)=>{
    ...
    try{
      setLoading(true)
      const prompt=`Write an article about ${input} in ${selectLength.text} words`
      ...
      <form onSubmit={onsubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
          ...
          <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
            {articleLength.map((item,index)=>(
              <span onClick={()=>setSelectedLength(item)} className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectLength.text===item.text ? 'bg-blue-50 text-blue-700' : 'text-gray-500 border-gray-300'}`} key={index}>{item.text}</span>
            ))}
          </div>
          <br />
